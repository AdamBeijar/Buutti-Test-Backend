...
def books(request): #handles all requests to /api/books
    try:
        if request.method == 'GET':
            if ("title" in request.GET and request.GET["title"] == "") or ("author" in request.GET and request.GET["author"] == "") or ("year" in request.GET and request.GET["year"] == "" and type(request.GET["year"]) != int):
                #checks if any of the parameters are empty
                return HttpResponse(status=400)
            if "author" in request.GET and "publisher" in request.GET and "year" in request.GET:
            ...
            elif "author" in request.GET and "publisher" in request.GET:
                #checks if author and publisher are provided
                currentBooks = list(Books.objects.filter(author=request.GET["author"], publisher=["publisher"]).values())
            elif "author" in request.GET and "year" in request.GET:
                #checks if author and year are provided
                currentBooks = list(Books.objects.filter(year=request.GET["year"], author=["author"]).values())
            elif "publisher" in request.GET and "year" in request.GET:
                #checks if publisher and year are provided
            ...
            try: #checks if the request body is valid json
                ...
                return HttpResponse(status=400)
            if "title" in jsonBody and "author" in jsonBody and "year" in jsonBody and jsonBody["title"] and jsonBody["author"] and jsonBody["year"] and type(jsonBody["year"]) == int: 
                #checks if all required fields are provided and are valid
                if "publisher" in jsonBody and jsonBody["publisher"] == "": 
                    #checks if publisher is empty
                    return HttpResponse(status=400)
                if doesBookExist(jsonBody): #checks if the book already exists
                    return HttpResponse(status=400)
                if "publisher" in jsonBody and "description" in jsonBody: 
                    #checks if publisher and description are provided
...
def doesBookExist(jsonBody): #checks if a book already exists
    currentBooks = Books.objects.filter(title=jsonBody["title"], author=jsonBody["author"], year=jsonBody["year"]) 
    #gets all books with the same title, author, and year
